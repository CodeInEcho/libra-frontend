...
import { offerType, abbreviateWalletAddress } from "@/lib/utils"
...
export default function IndexPage({ params }: { params: { offerId: string } }) {
    ...
    const [offerInfo, setOfferInfo] = useState({
        ...
        seller: { wallet_address: '', twitter: '', seller_items: 0, completion_rate: 0, seller_qualification: 'NORMAL', seller_level: 1 }
    });
            ...
            <div className="flex">
                ...
                <div className="border rounded-md p-4 w-[280px] flex flex-col items-center ml-[40px]">
                    <div className="flex flex-col my-6">
                        ...
                        <div className="leading-10">{ abbreviateWalletAddress(offerInfo.seller.wallet_address) }</div>
                    ...
                    <ul className="flex text-center border-y border-dashed w-full mt-[30px] py-[10px]">
                        <li className="flex-1">
                            <div className="text-sm">{offerInfo.seller.seller_items}</div>
                            <div className="text-[#6A6A6A] text-xs">Items</div>
                        </li>
                        <li className="flex-1">
                            <div className="text-sm">{offerInfo.seller.completion_rate * 100}%</div>
                            <div className="text-[#6A6A6A] text-xs">FulfRate</div>
                        </li>
                    </ul>
                </div>
            </div>
