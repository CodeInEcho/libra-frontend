        ...
        <div>
            <ul className={`${type === 'multiple' ? 'flex items-center justify-start' : 'inline-block'} mt-2 bg-[#F5F5F5] p-4 rounded-md  gap-2 flex-wrap`}>
                {[...props.newImages]
                    .map((image) => (
                        <div key={image.id}>
                            <li className={`relative w-36 h-36 ${props.className}`}>
                                <img
                                    src={image.url}
                                    alt={image.alt ?? ""}
                                    className={`object-cover w-36 h-36 rounded-md ${props.className}`}
                                />
                                <button
                                    type="button"
                                    onClick={() => {
                ...
                {(type === 'multiple' || (type !== 'multiple' && props.newImages.length === 0)) && (
                    <div
                        {...getRootProps()}
                        className={`border-border border-2 rounded-md border-dashed w-36 h-36 ${props.className}`}
                    >
